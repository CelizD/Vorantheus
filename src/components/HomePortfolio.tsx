...
export default function HomePortfolio() {
  return (
    <section className="section-mid border-t border-white/[0.06]">
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 pb-24">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {featured.map((item, i) => (
            <motion.div
              ...
              className="group rounded-3xl overflow-hidden bg-white/[0.02] border border-white/[0.07] hover:border-white/[0.18] transition-all duration-300"
            >
              {/* Image area */}
              <div className={`relative aspect-[16/9] bg-gradient-to-br ${gradientTints[item.imageColor] || 'from-[#0B84F3]/40 to-[#7C3AED]/30'}`}>
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <span className="text-4xl font-black text-white/80 tracking-wide">{item.title}</span>
                </div>
              ...
              <div className="p-7 flex items-start justify-between gap-4">
                ...
                <div className="flex flex-wrap gap-1.5 shrink-0 max-w-[120px] justify-end">
                  {item.technologies.slice(0, 2).map((tech) => (
                    <span key={tech} className="px-2 py-0.5 rounded-md bg-white/[0.04] border border-white/[0.06] text-white/40 text-[11px] whitespace-nowrap">
                      {tech}
                    </span>
                  ))}
                </div>
        ...
        <div className="sm:hidden text-center mt-8">
          <Link href="/portafolio" className="inline-flex items-center gap-1.5 text-sm text-[#0B84F3] hover:text-white transition-colors duration-200">
